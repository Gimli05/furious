...
/**
 * A szkeleton vezérléséért felelös osztály, mely kezeli a felhasználóval való kommunikációt és az alapján vezérli a programot.
 * A szkeleton tesztelésére 2 teljes és 6 további egyszerüsített teszt pályát hoztunk létre. A hat rövid pálya teljes játék indítására nem alkalmas.
 */
public class Main {
	public static void Amain(String[] args) throws IOException{
		 ...
		 */
		 ...
		 /**
		  * Ez a boolean változó felel azért, hogy a föciklus ami a teszteket tartalmazza folyamatosan fusson egészen
		  * addig amíg az értékét át nem állítják false-ba
		  */
		 boolean run = true;
		 
		 /* A skeleton tesztelö föciklusa. Folyamatosan fut, amíg a run értékét át nem írják false-ba. Ezt a kilépésre
		  * szolgáló billentyü lenyomásával lehet parancsba adni.
		  */
		while(run){
			
			/* A program elöször felkínálja a felhasználónak a teszteseteket amiket
			 * az alább feltüntetett billenytük lenyomásával lehet elérni. 
			...
			System.out.println("\n Válassz az alábbi lehetöségek közül:"
					+ "\n 1: Új játék indítása"
			 ...
			 * Az egyes billentyű lenyomásának hatására a szkeleton tesztelő program elindítja az "Új játék indítása" tesztesetet. 
			 * A kettes billentyű lenyomásának hatására a szkeleton tesztelő program elindítja az "Alagútszáj aktiválása" tesztesetet. 
			 * A hármas billentyű lenyomásának hatására a szkeleton tesztelő program elindítja az "Alagútszáj deaktiválása" tesztesetet. 
			 * A négyes billentyű lenyomásának hatására a szkeleton tesztelő program elindítja a "Váltó állítása" tesztesetet. 
			 * Az ötös billentyű lenyomásának hatására a szkeleton tesztelő program elindítja a "Vonatok léptetése" tesztesetet. 
			 * A hatos billentyű lenyomásának hatására a szkeleton tesztelő program elindítja az "Új vonat érkezése" tesztesetet. 
			 * A hetes billentyű lenyomásának hatására a szkeleton tesztelő program elindítja az "Győzelem" tesztesetet. 
			 * A nyolcas billentyű lenyomásának hatására a szkeleton tesztelő program futása megszakad.
			 */
			
			switch(Integer.parseInt(scanner.nextLine())){
			/* Először is intté kasztolom a bemenetet.*/
				 ...
				 * induljon el. */
					switch(Integer.parseInt(scanner.nextLine())){
						...
						/* Ez a programrész akkor fut le, ha a felhasználó az első pályát választja. Létrejön egy GameController. */
...
/**LONG TESZTEL--------------------------*/						
						 ...
						 * Ilyen pálya ugyanis nincs. Ekkor a program visszatér a főmenübe. */
				...
				/* Ez a teszteset alagútszáájak lérehozását teszi lehetővé a felhasználó számára. 
				 * Először megkérdezi, hány alagútszáj van nyitva jelenleg a pályán. Egyszerre csak két  
				...
				switch (Integer.parseInt(scanner.nextLine()))
				{
					case 0: System.out.println("Első alagútszáj aktiválása...");
					...
					gc21.skeletonTesterActivateTunnelEntrance(1);/* Aktiváljuk az első alagútszájat. */
					break;
					
					case 1: System.out.println("Első és második alagútszáj aktiválása...");
					...
					gc22.skeletonTesterActivateTunnelEntrance(1);/* Aktiváljuk az első alagútszájat. */				
					break;
					
					/* Amennyiben a felhasználó nem az 1-es vagy a 2-es gombot nyomta meg, egyik fenti teszteset
					 * sem fog lefutni. A szkeleton tesztelő visszatér a főmenübe.
					 */
				 ...
				 * Értelemszerűen kettőnél több aktív alagútszáj nem létezhet egy pályán egyszerre, ugyanis
				...
				switch (Integer.parseInt(scanner.nextLine()))
				{
					 ...
					 * Ebben az esetben csak egyszerűen deaktiválódik az alagútszáj.
					 */
						GameController gc31 = new GameController();
						gc31.startNewGame(1);  /* elindítjuk a gameControllert az 1-es pályával-  */
						gc31.skeletonTesterActivateTunnelEntrance(1);/* Aktiváljuk az első alagútszájat. */
					 ...
					 * pályán két alagútszáj volt aktív. Ebben az esetben az alagút a két alagútszáj között megszűnik, 
					 * és eltűnik az alagútszáj.
					 */
						...
						gc32.skeletonTesterDeActivateATunnelEntrance(); /* Deaktiváljuk az egyik alagútszájat mely során megszűnik az alagút. */
					break;
					
					/* Ha a felhasználó nem 1-et vagy 2-t adott meg bemenetként akkor a fenti 
					 * tesztesetek közül egyik se fut le. A program visszalép a főmenübe.
					 */
				...
				switch (scanner.nextLine().charAt(0))
				{
					 ...
					 * szeretnénk állítani. Ez majd GUI-n eventekkel történik majd a jövőben. Mivel a váltó alaphelyzetből indul,
					 * ehhez előbb ki kell billenteni az alternatív helyzetbe, majd onnan vissza.
					 */
					GameController gc41 = new GameController();
					gc41.startNewGame(1);  /* elindítjuk a gameControllert az 3-as pályával  */
					gc41.skeletonTesterSwitchASwitch(); /* Beállítjuk a váltót, hogy a feladat kiírásnak megfelelő alaphelyzetben álljon. */
					 ...
					 * és mi szeretnénk a másik állapotába helyezni. Ez majd GUI-n eventekkel történik majd a jövőben.
					 */
				 ...
				 * A vonat 5 különböző sínre léphet. Ezek a következők:
				 * Az egyes billentyű lenyomásának hatására a léptetést szimuláló teszteset sínre lépteti a vonatot. 
				 * A kettes billentyű lenyomásának hatására a léptetést szimuláló teszteset váltóra lépteti a vonatot. 
				 * A váltó egy sepciális sín
				 * A hármas billentyű lenyomásának hatására a léptetést szimuláló teszteset alagútszájra lépteti a vonatot. 
				 ...
				 * A négyes billentyű lenyomásának hatására a léptetést szimuláló teszteset megállóra lépteti a vonatot. 
				 * A megálló is egyfajta speciális sín ami színnel rendelkezik
				 * Az ötös billentyű lenyomásának hatására a léptetést szimuláló teszteset belépési pontra lépteti a vonatot.
				 ...
				 * Az ötös billenytű egy speciális teszteset, ugyanis ez nem sínhez köthető, hanem azt szimulálja
				...
				switch (Integer.parseInt(scanner.nextLine()))
				{
					...
					/*Ez a programrész fut le akkor, ha a vonat egy egyszerű sínre lép. */
					
						ArrayList<Color>cabinColors = new ArrayList<Color>(); /* Létrehozunk egy színlistát. Ebből fog felépülni a vonat. */
					...
					case 2: System.out.println("A vonat váltóra lép. A váltó alaphelyzetben (i) vagy az elternatív helyzetben (n) van? [i, n]");
						switch (scanner.nextLine().charAt(0)){
							...
							ArrayList<Color>cabinColors21 = new ArrayList<Color>(); /* Létrehozunk egy színlistát. Ebből fog felépülni a vonat. */
							...
							ArrayList<Color>cabinColors22 = new ArrayList<Color>();/* Létrehozunk egy színlistát. Ebből fog felépülni a vonat. */
					...
					switch (scanner.nextLine().charAt(0))
					{
						...
						case 'i': System.out.println("Hány aktív alagútszáj van a pályán? [1, 2]"); 
							...
							ArrayList<Color>cabinColors31 = new ArrayList<Color>();/* Létrehozunk egy színlistát. Ebből fog felépülni a vonat. */
							...
							gc531.skeletonTesterSwitchATunnelEntrance(); /* váltunk egyet az alagútszájon lévő váltón. */ 
							 ...
							 */
							ArrayList<Color>cabinColors32 = new ArrayList<Color>();/* Létrehozunk egy színlistát. Ebből fog felépülni a vonat. */
						...
						ArrayList<Color>cabinColors33 = new ArrayList<Color>();/* Létrehozunk egy színlistát. Ebből fog felépülni a vonat. */
					...
					/* Ha a vonat megállóra lép, az első kérdés, hogy utaznak-e a vonatban. Ha nem, akkor a vonat
					 * egyszerűen áthalad a megállón. Ha van olyan kabin ami nem üres akkor ha az első 
					 * nem üres vagon színe megegyezik a megállóéval, leszállnak az utasok arról a vagonról.
					 */
						switch (scanner.nextLine().charAt(0))
						{
							case 'I':
							case 'i': System.out.println("Milyen színű az első nem üres vagon? Piros, zöld vagy kék lehet. [r, g, b]"); 
							//* Ebben tároljuk a vagon színét amit a felhasználó megad. */
							char vagonSzin = scanner.nextLine().charAt(0);		
							Color cabColor = null; /* Ilyen színű lesz majd a vagon. */
														
							if(vagonSzin == 'r' || vagonSzin == 'R'){ /* ellenőrizzük a színeket. Ha piros, akkor nem fognak leszállni utasok, és megadjuk a cabColort pirosnak. */
								...
								System.out.println("A vagonok és a megállók csak 'r', 'g' vagy 'b' színűek lehetnek");
								break;
							}
							
							
							ArrayList<Color>cabinColors41 = new ArrayList<Color>();/* Létrehozunk egy színlistát. Ebből fog felépülni a vonat. */
						...
						ArrayList<Color>cabinColors42 = new ArrayList<Color>();/* Létrehozunk egy színlistát. Ebből fog felépülni a vonat. */
						...
						/* a program visszalép a főmenübe, ha nem kap érvényes bemenetet arra a kérdésre
					 ...
					 */
					ArrayList<Color>cabinColors5 = new ArrayList<Color>(); /* Létrehozunk egy színlistát. Ebből fog felépülni a vonat. */
					...
					ArrayList<Color>cabinColors6 = new ArrayList<Color>();/* Létrehozunk egy színlistát. Ebből fog felépülni a vonat. */
					...
					gc56.skeletonTesterMakeTrainsMove(); /* léptetjük, melynek hatására belép a játéktérbe és ütközik az előbb hozzáadott vonattal. */
					break;	
					
					/* Ha a felhasználó nem a várt kimenetet adja (vagyis nem 1, 2, 3,4 , 5, 6 számok valamelyikét
					 * akkor a tesztelő program kilép a főmenübe.
					 */
				 ...
				 * Először lekérdezi a felhasználótól a program, hogy hány vagon legyen a vonatban.
				 */
				...
				/*
				 * Ezek a booleanok azért kellenek, mert ha már elkezdődik a vonat vagonjainak felkonfigurálása,
				 * akkor ha a felhasználó nem létező színt írna be vagy arra a kérdésre, hogy a vagonban
				 * vannak-e érvénytelen választ adna, akkor a vagon érvénytelen paraméterekkel lenne inicializálva.
				 * Ezt elkerülendő, két while ciklus addig nem engedi tovább a program futását, amíg a 
				...
				for(int i = 0; i < vagonSzam; i++)
				{
					...
					{
						System.out.println("Milyen színű legyen a " + (i+1) + ". vagon? [r, g, b]");
						switch (scanner.nextLine().charAt(0))
						{
							case 'R':
							case 'r': System.out.println("Piros a(z) " + (i+1) + ". vagon"); 
							/* Ha egy érvényes színt ad meg a felhasználó, akkor kilép ebből a while
							 * ciklusból Ellenkező esetben, megint azt kérdezi tőle a program, milyen színű legyen
							 * ez a vagon.
							 */
				 ...
				 */
				case 7: System.out.println("Győzelem. Melyik pályán volt? [1, 2]");
				switch (Integer.parseInt(scanner.nextLine())){
					case 1: System.out.println("Győzelem az első pályán...");
					/* Ha a játékos az első pályát nyeri meg akkor a játék a második pályát elkezdi. */
					...
					break;
					case 2: System.out.println("Győzelem a második pályán");
			...
			/*Ha olyan tesztesetet indítana  a játékos ami nem is létezik, akkor a főmenüben marad. */
